...
class Student:
    def __init__(self, root):
        ...
        #bg image
        img3=Image.open(r"C:\Users\PC\Desktop\Face_recognition_system_ DBMS\images\bg_img.jpeg") 
        img3=img.resize((1530,710),Image.ANTIALIAS)               
        self.photoimg3=ImageTk.PhotoImage(img3)

        bg_img=Label(self.root,image=self.photoimg)
        bg_img.place(x=-10,y=80,width=1530,height=710)
        ...
        img4_left=Image.open(r"C:\Users\PC\Desktop\Face_recognition_system_ DBMS\images\student_details.jpg") 
        img4_left=img4_left.resize((720,130),Image.ANTIALIAS)               
        self.photoimg4_left=ImageTk.PhotoImage(img4_left)
        ...
        Right_frame.place(x=720,y=10,width=720,height=580)

        scroll_x=ttk.Scrollbar(Right_frame,orient=HORIZONTAL)
        scroll_y=ttk.Scrollbar(Right_frame,orient=VERTICAL)

        self.Right_frame=ttk.Treeview(Right_frame,xscrollcommand=scroll_x.set,yscrollcommand=scroll_y.set)

        scroll_x.pack(side=BOTTOM,fill=X)
        scroll_y.pack(side=RIGHT,fill=Y)
        scroll_x.config(command=self.Right_frame.xview)
        scroll_y.config(command=self.Right_frame.yview)

        img_right=Image.open(r"C:\Users\PC\Desktop\Face_recognition_system_ DBMS\images\face_recognition2.jpeg") 
    ...
    def add_data(self):
        if self.var_dep.get()=="Select Department" or self.var_std_name.get()=="" or self.va_std_id.get()=="":
            ...
            try:
                conn=mysql.connector.connect(host="localhost",username="root",password="maneeshkm1234",database="face_recognizer")
    ...
    def delete_data(self):
        if self.va_std_id.get()=="":
            ...
            try:
                delete=messagebox.askyesno("Student Delete Page","Do you wnat to delete this student",parent=self.root)
                if delete>0:
                    conn=mysql.connector.connect(host="localhost",username="root",password="maneeshkm1234",database="face_recognizer")
                    my_cursor=conn.cursor()
